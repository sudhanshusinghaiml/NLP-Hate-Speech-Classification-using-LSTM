...
from hateSpeechClassification.entity.artifact_entity import ModelEvaluationArtifacts, ModelTrainingArtifacts, DataTransformationArtifacts
from hateSpeechClassification.entity.config_entity import ModelEvaluationConfig


class ModelEvaluation:
    def __init__(self, model_evaluation_config: ModelEvaluationConfig, model_training_artifacts: ModelTrainingArtifacts, data_transformation_artifacts: DataTransformationArtifacts):
        """
        :param model_evaluation_config: Configuration for model evaluation model = model_architecture.get_model()
        
        :param model_trainer_artifacts: Output reference of model trainer artifact stage

        :param data_transformation_artifacts: Output reference of data transformation stage artifacts
    ...
    def get_best_model_from_gcloud(self) -> str:
        ...
        """
        try:
            logging.info("Inside get_best_model_from_gcloud method of Model Evaluation class")

            os.makedirs(self.model_evaluation_config.BEST_MODEL_DIR_PATH, exist_ok=True)

            self.gcloud.download_data_from_gcloud(self.model_evaluation_config.BUCKET_NAME,
            ...
            best_model_path = os.path.join(self.model_evaluation_config.BEST_MODEL_DIR_PATH,
                                           self.model_evaluation_config.MODEL_NAME)
            logging.info("Fetched best model using get_best_model_from_gcloud method of Model Evaluation class")
    ...
    def evaluate_model_performance(self):
        ...
        """
        try:
            logging.info("Inside evaluate_model_performance method of Model Evaluation class")
            ...
            loaded_model=keras.models.load_model(self.model_training_artifacts.trained_model_path)
            ...
            test_sequences_matrix = pad_sequences(test_sequences, maxlen=MAXIMUM_LENGTH)
            logging.info(f"----------{test_sequences_matrix}------------------")

            logging.info(f"-----------------{x_test.shape}--------------")
            logging.info(f"-----------------{y_test.shape}--------------")
            accuracy = loaded_model.evaluate(test_sequences_matrix, y_test)
            ...
            for prediction in lstm_prediction:
                if prediction[0] < 0.5:
                    ...
                    res.append(1)
            logging.info(f'Confusion Matrix is - \n{confusion_matrix(y_test,res)}')
            logging.info(f"Classification Report is {classification_report(y_test,res)} ")
            return accuracy
        except Exception as e:
            raise CustomException(e, sys) from e
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifacts:
        ...
        """
        logging.info("Inside initiate_model_evaluation method of ModelEvaluation class")
        try:

            logging.info("Loading currently trained model")
            trained_model=keras.models.load_model(self.model_training_artifacts.trained_model_path)
            with open('tokenizer.pickle', 'rb') as handle:
                load_tokenizer = pickle.load(handle)

            trained_model_accuracy = self.evaluate_model_performance()
            ...
            if os.path.isfile(best_model_path) is False:
                ...
                best_model_accuracy= self.evaluate_model_performance()
